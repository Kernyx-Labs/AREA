...
function buildWorkflowData() {
  ...
  return {
    ...
    trigger: {
      ...
      event: trigger.value.eventId || trigger.value.eventName,  // Use eventId if available, fallback to eventName
      config: trigger.value.config || {}
    },
    action: {
      service: actions.value[0].serviceName,
      event: actions.value[0].eventId || actions.value[0].eventName,  // Use eventId if available
      config: actions.value[0].config || {}
    }
  }
}
