...
> ⚠️ **Important**: This project is longer than your usual ones. It will be **essential** for the success of this project that you put in place real project management and not rush headlong.
...
> ⚠️ **Critical**: No business process will be performed on the **web & mobile** client side, which only serves as a user interface and redirects requests from/to the **application server**.

> 💡 **Accessibility**: While developing the web client and mobile client, consider accessibility guidelines and best practices. Make your application usable by as many people as possible, including those with disabilities. [Click here for more information](https://www.w3.org/WAI/)
...
> ⚠️ **Important**: The **application server** is only exploited for web and/or mobile clients, which will have to expose all of its functionalities through a **REST API**.
...
> ⚠️ **Note**: In the case where one of the two web or mobile clients offers less functions than the other, this calculation will be based on the least successful client.
...
The client asks non-identified users to register via an online form. Take inspiration by what you already know about this step (e.g., fill in an email address, registration via a third party service like Google, Facebook, X, etc.).
...
> 💡 **Bonus**: An administration section would be useful to manage site users.
...
- A method of **identifying** users via OAuth2 (e.g., Google / X / Facebook / etc.). In this case, the client processes itself this identification and warns the application server if successful.

> 💡 **Important**: Regarding the method of identification, remember to connect the third party account to a system user.
...
In this part, it will therefore be necessary to ask users to subscribe to these **services** (e.g., from their profile page the user links their X / Google account / etc via an authentication OAuth2).

When it is necessary, the client will have to manage the identification before transmitting to the application server the subscription to this **service** (e.g., the user subscribes to the Facebook service which requires them to log in via OAuth2).
...
For the Action **"A project P is to be completed in less than H hours"**:
...
> 💡 **Reference**: Refer to the [IFTTT](https://ifttt.com) or [Zapier](https://zapier.com) website for a better understanding of this feature if necessary.
...
> ⚠️ **Important**: The **mobile client** must provide a way to configure the network location of the **application server**.
...
> ⚠️ **Critical**: No business model process will be carried out on the **web & mobile** client side. This only serves as a user interface and redirects requests from/to the **application server**.
...
> ⚠️ **Please read the following points carefully**
...
> 💡 **Dependency**: The **client_web** depends on **client_mobile** AND **server**. For more information, see the documentation about [depends_on](https://docs.docker.com/compose/compose-file/#depends_on)
...
- The services **client_mobile** and **client_web** will share a [common volume](https://docs.docker.com/storage/volumes/)
- The **client_mobile** service will edit the associated binary and put it on the common volume with the **client_web**
- The **server** service will run by exposing the [port](https://docs.docker.com/compose/compose-file/#ports) `8080`
- The **server** service will respond to the request `http://localhost:8080/about.json` (see File about.json)
- The **client_web** service will run by exposing the [port](https://docs.docker.com/compose/compose-file/#ports) `8081`
...
- **server.current_time** indicates the server time in the [Epoch Unix Time Stamp](https://www.epochconverter.com/) format
...
1. Provide a comprehensive **README.md** that details your server's API. This must include clear installation and setup instructions explaining how to install dependencies, configure the environment, and launch the project locally for development purposes.

2. Include essential diagrams like class and sequence diagrams for the most important system components.

3. Additionally, you must include a **HOWTOCONTRIBUTE.md**. This file should serve as a practical guide for extending the project's functionality. It must detail the process for adding new elements such as services, actions, reactions, or other core features.
...
> ⚠️ **Important**: All requested `.md` documents must be written in [markdown format](https://www.markdownguide.org/).
