package com.area.server.service.integration;

import java.util.List;

/**
 * Defines a reaction (action) that a service integration can perform.
 * Reactions are actions executed in response to triggers (e.g., "Send Discord message").
 */
public class ReactionDefinition {
    private String id;
    private String name;
    private String description;
    private List<FieldDefinition> configFields;

    public ReactionDefinition() {
    }

    public ReactionDefinition(String id, String name, String description, List<FieldDefinition> configFields) {
        this.id = id;
        this.name = name;
        this.description = description;
        this.configFields = configFields;
    }

    public String getId() {
        return id;
    }

    public void setId(String id) {
        this.id = id;
    }

    public String getName() {
        return name;
    }

    public void setName(String name) {
        this.name = name;
    }

    public String getDescription() {
        return description;
    }

    public void setDescription(String description) {
        this.description = description;
    }

    public List<FieldDefinition> getConfigFields() {
        return configFields;
    }

    public void setConfigFields(List<FieldDefinition> configFields) {
        this.configFields = configFields;
    }
}
